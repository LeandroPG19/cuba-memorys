...
pub fn tool_definitions() -> Vec<Value> {
    vec![
        ...
        tool_def(
            "cuba_faro",
            "Search memory BEFORE answering to ground responses. Returns grounding scores. Mode 'verify' checks claims against evidence (confidence: verified/partial/weak/unknown). Session-aware: boosts results matching active session goals.",
            serde_json::json!({
                "type": "object",
                "properties": {
        ...
        tool_def(
            "cuba_zafra",
            "Memory maintenance: decay (FSRS adaptive), prune (remove low-importance), merge (deduplicate), summarize (compress observations), pagerank (personalized importance), find_duplicates, export, stats.",
            serde_json::json!({
                "type": "object",
                "properties": {
