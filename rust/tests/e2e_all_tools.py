...
# Configuration
BINARY_PATH = "/home/lpg/Work/Programacion/ScripTs/cuba-memorys/rust/target/release/cuba-memorys"
...
def invoke_mcp(request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            print(f"  [ERROR] MCP invocation failed (exit={result.returncode}): {stderr_preview}")
            return None
...
def test(tool_name: str, action: str, request: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    """
...
def test_cuba_alma():
    ...
    # Get updated
    result = test("cuba_alma", "get_renamed", {
        "action": "get",
        "name": new_name,
    })
    if result is None:
...
def test_cuba_cronica():
    ...
    test("cuba_alma", "create_cronica_entity", {
        "action": "create",
        "name": entity_name,
        "entity_type": "concept",
    })
    ...
    # Add another
    result = test("cuba_cronica", "add_second", {
        "action": "add",
        "entity_name": entity_name,
    ...
    result = test("cuba_cronica", "batch_add", {
        "action": "batch_add",
        "entity_name": entity_name,
        "observations": [
            {
    ...
    result = test("cuba_cronica", "list_all", {
        "action": "list",
        "entity_name": entity_name,
    })
    if result is None:
    ...
    # List final
    result = test("cuba_cronica", "list_final", {
        "action": "list",
        "entity_name": entity_name,
    })
    if result is None:
...
def test_cuba_faro():
    ...
    result = test("cuba_faro", "search_hybrid", {
        "query": "_e2e_cronica",
        "mode": "hybrid",
        "limit": 5,
    })
    ...
    # Verify mode
    result = test("cuba_faro", "search_verify", {
        "query": "_e2e_cronica",
        "mode": "verify",
    })
    if result is None:
...
def test_cuba_puente():
    ...
    # Create entities
    test("cuba_alma", "create_A", {
        "action": "create",
        "name": entity_a,
        "entity_type": "concept",
    })
    test("cuba_alma", "create_B", {
        "action": "create",
        "name": entity_b,
        "entity_type": "concept",
    })
    ...
    # Create relation
    result = test("cuba_puente", "create_relation", {
        "action": "create",
    ...
    # Traverse from A
    result = test("cuba_puente", "traverse", {
        "action": "traverse",
    ...
    # Delete relation
    result = test("cuba_puente", "delete_relation", {
        "action": "delete",
        "from_entity": entity_a,
        "to_entity": entity_b,
    })
...
def test_cuba_alarma():
    ...
    result = test("cuba_alarma", "report", {
...
def test_cuba_remedio():
    ...
    # Resolve it
    result = test("cuba_remedio", "resolve", {
        "error_id": error_id,
...
def test_cuba_expediente():
    ...
    # Search for errors
    result = test("cuba_expediente", "search", {
        "query": "E2ETestError",
    })
    if result is None:
        return False
    ...
    result = test("cuba_expediente", "proposed_action", {
        "query": "E2ETestError",
    ...
    # resolved_only
    result = test("cuba_expediente", "resolved_only", {
        "query": "E2ETestError",
        "resolved_only": True,
    })
    if result is None:
...
def test_cuba_jornada():
    ...
    # Start session
    result = test("cuba_jornada", "start", {
        "action": "start",
        "name": session_name,
    ...
    # Get current
    result = test("cuba_jornada", "current", {
        "action": "current",
    })
    if result is None:
        return False
...
def test_cuba_decreto():
    ...
    # Record a decision
    result = test("cuba_decreto", "record", {
        "action": "record",
    ...
    # Query for it
    result = test("cuba_decreto", "query", {
        "action": "query",
...
def test_cuba_zafra():
    ...
    # PageRank
    result = test("cuba_zafra", "pagerank", {
        "action": "pagerank",
    })
    if result is None:
        return False
    ...
    # Find duplicates
    result = test("cuba_zafra", "find_duplicates", {
...
def test_cuba_forget():
    ...
    test("cuba_alma", "create_helper", {
        "action": "create",
        "name": helper,
        "entity_type": "concept",
    })
    test("cuba_puente", "create_forget_relation", {
        "action": "create",
    ...
    result = test("cuba_forget", "forget", {
        "action": "forget",
        "entity_name": entity_name,
        "confirm": True,
    })
...
def cleanup():
    ...
    for entity in entities_to_cleanup:
        ...
        test("cuba_alma", f"cleanup_{entity}", {
            "action": "delete",
            "name": entity,
        })
