...
| `CUBA_PANEL_PUBLIC` | unset | Without it, `/panel` refuses any request carrying `CF-Connecting-IP`, `X-Forwarded-For` or `X-Real-IP` — the signature of having come through a tunnel or proxy. The Cloudflare tunnel connects to `127.0.0.1`, so the client address is loopback either way and only the forwarding header tells the two apart. Set to `1` to publish the panel deliberately |
